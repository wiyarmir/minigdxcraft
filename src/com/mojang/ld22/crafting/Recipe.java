...
public abstract class Recipe implements ListItem {
	...
	public void renderInventory(PortScreen screen, int x, int y) {
		screen.render(x, y, resultTemplate.getSprite(),
				resultTemplate.getColor(), 0);
		int textColor = canCraft ? Color.get(-1, 555, 555, 555) : Color.get(-1,
				222, 222, 222);
		PortFont.draw(resultTemplate.getName(), screen, x + 8, y, textColor);
	}
