...
public class LevelGen {
	...
	private static byte[][] createTopMap(int w, int h) {
		...
		/*
		 * for (int i = 0; i < w * h / 2800; i++) { int xs = random.nextInt(w);
		 * int ys = random.nextInt(h); for (int k = 0; k < 10; k++) { int x = xs
		 * + random.nextInt(21) - 10; int y = ys + random.nextInt(21) - 10; for
		 * (int j = 0; j < 100; j++) { int xo = x + random.nextInt(5) -
		 * random.nextInt(5); int yo = y + random.nextInt(5) -
		 * random.nextInt(5); for (int yy = yo - 1; yy <= yo + 1; yy++) for (int
