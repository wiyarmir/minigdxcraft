...
public class CloudTile extends Tile {
	...
	public void render(PortScreen screen, Level level, int x, int y) {
		...
		if (!u && !l) {
			if (!ul)
				...
				screen.render(x * 16 + 0, y * 16 + 0, 7 + 0 * 32,
						transitionColor, 3);
		} else
			screen.render(x * 16 + 0, y * 16 + 0, (l ? 6 : 5) + (u ? 2 : 1)
					* 32, transitionColor, 3);

		if (!u && !r) {
			if (!ur)
				screen.render(x * 16 + 8, y * 16 + 0, 18, col, 0);
			else
				screen.render(x * 16 + 8, y * 16 + 0, 8 + 0 * 32,
						transitionColor, 3);
		} else
			screen.render(x * 16 + 8, y * 16 + 0, (r ? 4 : 5) + (u ? 2 : 1)
					* 32, transitionColor, 3);
		...
		if (!d && !l) {
			if (!dl)
				...
				screen.render(x * 16 + 0, y * 16 + 8, 7 + 1 * 32,
						transitionColor, 3);
		} else
			screen.render(x * 16 + 0, y * 16 + 8, (l ? 6 : 5) + (d ? 0 : 1)
					* 32, transitionColor, 3);
		if (!d && !r) {
			if (!dr)
				screen.render(x * 16 + 8, y * 16 + 8, 19, col, 0);
			else
				screen.render(x * 16 + 8, y * 16 + 8, 8 + 1 * 32,
						transitionColor, 3);
		} else
			screen.render(x * 16 + 8, y * 16 + 8, (r ? 4 : 5) + (d ? 0 : 1)
					* 32, transitionColor, 3);
	}
			...
			if (tool.type == ToolType.shovel) {
				if (player.payStamina(5)) {
					...
					for (int i = 0; i < count; i++) {
						level.add(new ItemEntity(new ResourceItem(
								Resource.cloud), xt * 16 + random.nextInt(10)
								+ 3, yt * 16 + random.nextInt(10) + 3));
					}
					return true;
				}
			}
	 ...
	 * (ToolItem) item; if (tool.type == ToolType.pickaxe) { if
	 * (player.payStamina(4 - tool.level)) { hurt(level, xt, yt,
	 * random.nextInt(10) + (tool.level) * 5 + 10); return true; } } } return
	 * false; }
	 */
}
