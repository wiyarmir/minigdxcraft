...
public class SandTile extends Tile {
	...
	public void render(PortScreen screen, Level level, int x, int y) {
		int col = Color.get(level.sandColor + 2, level.sandColor,
				level.sandColor - 110, level.sandColor - 110);
		int transitionColor = Color.get(level.sandColor - 110, level.sandColor,
		...
		if (item instanceof ToolItem) {
			...
			if (tool.type == ToolType.shovel) {
				if (player.payStamina(4 - tool.level)) {
					...
					level.add(new ItemEntity(new ResourceItem(Resource.sand),
							xt * 16 + random.nextInt(10) + 3, yt * 16
									+ random.nextInt(10) + 3));
					return true;
				}
			}
