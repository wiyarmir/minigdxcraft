...
public class CraftingMenu extends Menu {
	...
	public void render(PortScreen screen) {
		PortFont.renderFrame(screen, "Have", 12, 1, 19, 3);
		PortFont.renderFrame(screen, "Cost", 12, 4, 19, 11);
		PortFont.renderFrame(screen, "Crafting", 0, 1, 11, 11);
		...
		if (recipes.size() > 0) {
			...
			PortFont.draw("" + hasResultItems, screen, xo + 8, 2 * 8,
			...
			for (int i = 0; i < costs.size(); i++) {
				...
				if (has > 99)
					has = 99;
				PortFont.draw("" + requiredAmt + "/" + has, screen, xo + 8, yo,
						color);
			}
		}
		// renderItemList(screen, 12, 4, 19, 11, recipes.get(selected).costs,
		// -1);
	}
}